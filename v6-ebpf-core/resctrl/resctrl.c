/*
 * resctrl.c -- V6 resctrl helper.
 *
 * Same operational model as V4: one mon_group per run, PIDs written into
 * the tasks file, counters summed across every mon_L3_* domain. The
 * difference is the API shape: V6 consumers want a group *handle* so the
